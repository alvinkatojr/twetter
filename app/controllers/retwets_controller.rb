class RetwetsController < ApplicationController
  ...
  # params[:retwet][:twet_id]. If the retwet us created successfully, a success notice
  # is set. Otherwise, an error notice is set. #smart_return is used to redirect appropriatelt.
  # 

  def create
    rt = current_user.retwets.create(retwet_params)
    if rt.valid? and rt.persisted?
      flash[:success] = "You retweeted: #{rt.twet.content}"
    else
  ...
  # DELETE /retwets/:id
  #
  # Responsible for un-retweting. Works by deletng the Retwet instance. The use of the
  ...
  # is appropriate. If the params[:retwet] does not exist or contains any key / value
  # pairs other then :twet_id, an error will not be raised and the page will return
